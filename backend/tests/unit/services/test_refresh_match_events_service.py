...
def test_fetch_match_events_for_match_returns_empty_list_when_api_has_no_events(mocker, api_payload):
    match = make_match(external_api_id=9001)
...
def test_refresh_match_events_completes_successfully_when_there_are_no_live_matches(mocker, db):
    ...
    complete_job = mocker.patch.object(refresh_match_events_service.refresh_jobs_repo, 'complete_job')
...
def test_refresh_match_events_skips_match_when_api_returns_no_rows(mocker, db):
    ...
    mocker.patch.object(refresh_match_events_service.refresh_jobs_repo, 'create_job', return_value=123)
    complete_job = mocker.patch.object(refresh_match_events_service.refresh_jobs_repo, 'complete_job')
    mocker.patch.object(
        refresh_match_events_service.matches_service,
        'get_live_matches',
...
def test_refresh_match_events_records_individual_match_failure_and_continues(mocker, db):
    ...
    assert fetch_events.call_args_list == [mocker.call(failing_match), mocker.call(succeeding_match)]
...
def test_refresh_match_events_marks_job_failed_and_reraises_when_live_match_lookup_fails(mocker, db):
    mocker.patch.object(refresh_match_events_service.refresh_jobs_repo, 'create_job', return_value=123)
    complete_job = mocker.patch.object(refresh_match_events_service.refresh_jobs_repo, 'complete_job')
    mocker.patch.object(
        refresh_match_events_service.matches_service,
